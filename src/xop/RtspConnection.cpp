	...
	, rtsp_response_(new RtspResponse)
{
	...
	});//当有数据从 RTSP 客户端通过 TCP socket 发送过来时的处理逻辑。用于处理 RTSP 协议的消息 
	...
	rtp_channel_->SetReadCallback([this]() { this->HandleRead(); });//处理 socked 上的读事件
...
bool RtspConnection::OnRead(BufferReader& buffer)
{
	...
	if (conn_mode_ == RTSP_SERVER) {//解析一条 RTSP 请求
