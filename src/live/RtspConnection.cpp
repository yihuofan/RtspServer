...
void RtspConnection::handleReadBytes(){
    ...
    if (!parseRequest()) // RTP over UDP
    {
        LOGE("parseRequest err");
        goto disConnect;
    }
    switch (mMethod) // 根据解析出的 RTSP 方法（OPTIONS、DESCRIBE 等）调用对应的处理函数
    {
    case OPTIONS: // 获取支持的方法
        if (!handleCmdOption())
            goto disConnect;
        break;
    case DESCRIBE: // 获取 SDP 描述信息
        if (!handleCmdDescribe())
            goto disConnect;
        break;
    case SETUP: // 建立媒体流传输通道（RTP/RTCP）
        if (!handleCmdSetup())
            goto disConnect;
        break;
    case PLAY: // 开始播放
...
{
    // 添加调试信息
    LOGI("parseRequest() buffer size: %d", mInputBuffer.readableBytes());
    LOGI("parseRequest() buffer content: [%.*s]",
         mInputBuffer.readableBytes(), mInputBuffer.peek());
    //解析第一行
