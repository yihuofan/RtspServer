...
TaskScheduler::TaskScheduler(int id)
	: id_(id)
	, is_shutdown_(false) 
	, wakeup_pipe_(new Pipe())//管道对象，用于线程唤醒
	, trigger_events_(new xop::RingBuffer<TriggerEvent>(kMaxTriggetEvents))//生产者-消费者
{
...
{
	char event[10] = { 0 };
	while (wakeup_pipe_->Read(event, 10) > 0);//循环读取直到没有数据为止
}
